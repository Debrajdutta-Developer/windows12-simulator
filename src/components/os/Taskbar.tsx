...
const APP_ICONS: Record<AppID, React.ReactNode> = {
  ...
  terminal: <TermIcon size={20} className="text-gray-700" />,
  settings: <SettingsIcon size={20} className="text-gray-600" />
};
...
const Clock = () => {
  ...
  if (!time) return <div className="w-16 h-8 animate-pulse bg-black/5 rounded" />;
...
export const Taskbar = () => {
  ...
  return (
    <div className="h-12 w-full fixed bottom-0 left-0 right-0 z-[9999] flex items-center justify-between px-2 bg-white/70 backdrop-blur-2xl border-t border-white/20">
      ...
      <div className="flex items-center gap-1 bg-white/20 p-1 rounded-xl border border-white/30 shadow-sm">
        ...
        {pinnedApps.map(app => {
          ...
          return (
            ...
            >
              <div className={cn("transition-transform group-active:scale-90", !isActive && "group-hover:-translate-y-0.5")}>
                {APP_ICONS[app.id]}
              </div>
              {isOpen && (
                <div className={cn(
        ...
        {windows.filter(w => !pinnedApps.some(p => p.id === w.id)).map(w => (
          ...
          >
            <div className="transition-transform group-active:scale-90 group-hover:-translate-y-0.5">
              {APP_ICONS[w.id]}
            </div>
            <div className={cn(
              "absolute bottom-0.5 left-1/2 -translate-x-1/2 h-1 rounded-full bg-blue-500 transition-all",
